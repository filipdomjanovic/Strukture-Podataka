...
int request(Position_I head_I) {
    ...
    printf("\nUnesite pocetni datum(dan, mjesec i godinu)\n");
    scanf("%d %d %d", &start.day, &start.month, &start.year);

    printf("\nUnesite konacni datum(dan, mjesec i godinu)\n");
    ...
    if (flag) {
        printf("Traženi proizvod nije pronaden\n");
        return 1;
    }

    printf("Kupljena kolicina je: %d\n", quantity);
    printf("Cijena te kolicine je: %lf\n", price);
